    ...
    view! {
        <div class=move || if collapsed.get() { "sticky top-0 h-screen w-8 bg-gray-800 text-white flex items-center justify-center" } else { "sticky top-0 h-screen w-64 bg-gray-800 text-white" }>
            {move || if collapsed.get() {
                // Collapsed View: A simple vertical bar with a right arrow
                view! {
                    <button class="text-white text-2xl" on:click=move |_| {
                        set_collapsed.update(|c| {
                            let new_value = !*c;
                            *c = new_value; // update the signal value
                        });
                    }>
                        "→"
                    </button>
                ...
                // Expanded View: Full sidebar content
                view! {
                    <>
                        ...
                        <button class="absolute top-4 right-4 text-white" on:click=move |_| {
                            set_collapsed.update(|c| {
                                let new_value = !*c;
                                *c = new_value;
                            });
